...
install:
	...
	if test -x "$(SYSTEMCTL)" && test -d "$(DESTDIR)$(SYSCONFDIR)/systemd/system"; then install -m0644 dash-listeners.service.app $(DESTDIR)$(SYSCONFDIR)/systemd/system/dash-listeners.service && $(SYSTEMCTL) daemon-reload; else echo "could not find systemd"; fi
